...
/// NSDocument-compatible SwiftUI document backed by a package bundle.
///
/// Package structure:
/// ```
...
/// ```
@Observable
final class InputForgeDocument: ReferenceFileDocument {
    // MARK: - State
    ...
    func snapshot(contentType: UTType) throws -> DocumentSnapshot {
        DocumentSnapshot(
            ...
            versions: versions,
            assetFiles: [:]  // Assets handled separately via FileWrapper
        )
    }
    ...
    func fileWrapper(snapshot: DocumentSnapshot, configuration: WriteConfiguration) throws -> FileWrapper {
        ...
        // Preserve existing asset files from the current file on disk
        if let existing = configuration.existingFile?.fileWrappers?[Self.assetsDirname]?.fileWrappers {
            for (name, wrapper) in existing {
                ...
                // Set preferred filename to keep original name
                if let added = assetsDir.fileWrappers?[name] {
                    added.preferredFilename = name
                }
            }
        }
        // Add any new asset data from snapshot
    ...
    func restoreVersion(_ snapshot: VersionSnapshot) {
